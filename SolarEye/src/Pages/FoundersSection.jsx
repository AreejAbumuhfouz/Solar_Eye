...
const FoundersSection = () => {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 text-center">
        <motion.h2
          className="text-3xl md:text-4xl font-bold mb-6 text-[#1C1F2D]"
          initial={{ scale: 0.9, opacity: 0 }}
          whileInView={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.5 }}
        >
        ...
        <motion.p
          ...
          transition={{ duration: 0.5, delay: 0.2 }}
        >
          We are four passionate women engineers from Jordan who are transforming solar
          infrastructure monitoring through innovative drone technology.
        </motion.p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {founders.map((founder, index) => (
            <motion.div
              key={founder.name}
              className="bg-white rounded-xl shadow-lg overflow-hidden transform transition-all hover:scale-105 hover:shadow-xl flex flex-col"
              initial={{ opacity: 0, scale: 0.9 }}
              whileInView={{
                opacity: 1,
                scale: 1,
                transition: { delay: index * 0.2, type: "spring", damping: 15, stiffness: 120 },
              }}
            >
              <div
                className="h-64 sm:h-56 md:h-64 bg-cover bg-center"
                style={{ backgroundImage: `url('${founder.image}')` }}
              />
              <div className="p-6 flex flex-col flex-1">
                <h3 className="text-2xl font-bold text-[#185B8D] mb-1">{founder.name}</h3>
